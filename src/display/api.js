...
//   const urlObj = new URL(src.docBaseUrl);
//   const params = new URLSearchParams(urlObj.search);
//   const baseEnv = params.get("baseEnv")?.toLocaleLowerCase();

//   console.log(baseEnv, "baseEnv");
//   if (baseEnv !== "dev") {
//     // Prevent Copy/Save/Print
...
//   // 🔹 Get encrypted params & env
//   const getParams = () => {
//     const pdfEncrypt = (value) => {
//       let now = new Date();
//       let date = Date.UTC(
//         now.getUTCFullYear(),
//         now.getUTCMonth(),
...
//       );
//       const encryptionKey = "B27AA05B9A2490D1AE59B33B45CFD4B0";
//       const initializationVector = date + ":" + "vg";
//       const encryptedData = encryptData(value, encryptionKey, initializationVector);
//       return { pass: encryptedData, iv: initializationVector };
//     };
...
//     const handleDecryption = (encryptedValue, iv) => {
//       const encryptionKey = "B27AA05B9A2490D1AE59B33B45CFD4B0";
...
//       const filePatient = handleDecryption(params.get("file"), params.get("salt"));
//       const salt = params.get("salt");
//       const token = params.get("token");
//       const baseEnv = params.get("baseEnv")?.toLocaleLowerCase();
//       const tenantId = params.get("tenantId");
//       const pdfEnv = params.get("pdfEnv");
//       const headers = JSON.parse(params.get("header"));
//       const fileDetails = pdfEncrypt(filePatient);

//       details = {
//         file: encodeURIComponent(fileDetails.pass),
//         salt: encodeURIComponent(fileDetails.iv),
//         token,
//         baseEnv,
//         tenantId,
//         pdfEnv,
...
//   const { docId } = task;
//   const detail = src.docBaseUrl ? getParams() : null;

//   let worker = src.worker instanceof PDFWorker ? src.worker : null;

//   if (!worker) {
//     const workerParams = { verbosity: src.verbosity, port: GlobalWorkerOptions.workerPort };
...
//   // 🔹 Instead of using NetworkStream, fetch PDF bytes directly
//   const fetchPDFBytes = async () => {
//     const renderUrl = details?.pdfEnv;
//     const finalUrl = details?.tenantId
//       ? `${renderUrl}?blobName=${details.file}&salt=${details.salt}&location=PATIENT_FILE&tenantId=${details.tenantId}`
//       : `${renderUrl}?blobName=${details.file}&salt=${details.salt}&location=PATIENT_FILE`;
...
//     console.log("Fetching PDF bytes from:", finalUrl);
...
//     // Restart PDF.js getDocument with byte array
//     return getDocument({
//       ...src,
//       data: pdfBytes,
//       url: null, // disable URL streaming
//       docBaseUrl: src.docBaseUrl,
//     });
//   } catch (error) {
//     console.error("Error in getDocument:", error);
//     task._capability.reject(error);
//     return task;
//   }
// }
// ✅ Keep reference to original PDF.js getDocument

/* At the top of api.js, add this if not already */
...
console.error("📥 Fetching PDF:", src);
console.log("window.location",window)
src.docBaseUrl=window?.location?.href;
  const urlObj = new URL(src.docBaseUrl);
  const params = new URLSearchParams(urlObj.search);
  const baseEnv = params.get("baseEnv")?.toLocaleLowerCase();
  ...
  const getParams = () => {
    ...
    try {
      console.error("📥 Fetching PDF1:", src);
      const urlObj = new URL(src.docBaseUrl);
      const params = new URLSearchParams(urlObj.search);
      const file = params.get("file");
      const filePatinet = handleDecryption(
        params.get("file"),
        params.get("salt")
      );
      const salt = params.get("salt");
      ...
      const pdfEnv = params.get("pdfEnv");
      const headers = JSON.parse(params.get("header"));
      const fileDetails = pdfEncrypt(filePatinet);
console.log("fileDetails",fileDetails)
      details = {
        file: encodeURIComponent(fileDetails.pass),
        salt: encodeURIComponent(fileDetails.iv), // vignesh need to change
  ...
  worker.promise
    .then(function () {
      ...
      const networkStreamPromise = new Promise(function (resolve) {
        console.log("detail?.baseEnv", details?.baseEnv);

        const renderUrl = details?.pdfEnv;

        console.log(details, renderUrl, "env");
        let networkStream;
        if (rangeTransport) {
          ...
          const createPDFNetworkStream = (params) => {
            ...
            // Ensure the httpHeaders include the Authorization token
            const headers = {
              ...params.httpHeaders,
              Authorization: `Bearer ${params.token}`, // Add your token here
              ...details.headers
            };

            return isValidFetchUrl(params.url)
              ? new PDFFetchStream({ ...params, httpHeaders: headers })
              : new PDFNetworkStream({ ...params, httpHeaders: headers });
          };
     
          if (fileId == details.file) {
          } else {
            fileId = details.file;
            networkStream = createPDFNetworkStream({
              url: details?.tenantId
                ? `${renderUrl}?blobName=${details.file}&salt=${details.salt}&location=PATIENT_FILE&tenantId=${details?.tenantId}`
                : `${renderUrl}?blobName=${details.file}&salt=${details.salt}&location=PATIENT_FILE`,
              length,
              httpHeaders,
